...
namespace dubu_man {
__device__ __host__ camera::camera(const camera_settings& settings) : settings(settings) {
  image_width     = settings.image_width;
  image_height    = static_cast<size_t>(fmax(static_cast<float>(image_width) / settings.aspect_ratio, 1.0f));
  ...
  const auto viewport_upper_left = m_center - settings.focus_dist * w - viewport_u * 0.5f - viewport_v * 0.5f;
...
__device__ ray camera::get_ray(size_t i, size_t j, curandState& rand_state) const {
  ...
  const auto pixel_sample =
      m_pixel00_loc + (offset.x + static_cast<float>(i)) * m_pixel_delta_u + (offset.y + static_cast<float>(j)) * m_pixel_delta_v;
