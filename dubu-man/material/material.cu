...
namespace dubu_man {
    ...
    __host__ __device__ material2 material2::make_lambertian(const color &albedo) {
        return {.type=material_type::lambertian, .lambertian = {.albedo = albedo}};
    }

    __host__ __device__ material2 material2::make_metal(const color &albedo, float fuzz) {
        return {.type=material_type::metal,
                .metal = {
                        .albedo = albedo,
                        .fuzz=fuzz}};
    }

    __host__ __device__ material2 material2::make_dielectric(float ior) {
        return {.type=material_type::dielectric, .dielectric = {.ior = ior}};
    }

    __device__ bool material2::scatter(const ray &r_in, const hit_record &rec, color &attenuation, ray &scattered,
    ...
    __device__ color const &material2::get_albedo(const hit_record &rec) const {
        if (type == material_type::lambertian) {
    ...
    __device__ bool
    material2::scatter_lambertian(const ray &r_in, const hit_record &rec, color &attenuation, ray &scattered,
                                  curandState &rand_state) const {
    ...
    __device__ bool material2::scatter_metal(const ray &r_in, const hit_record &rec, color &attenuation, ray &scattered,
                                             curandState &rand_state) const {
    ...
    __device__ bool
    material2::scatter_dielectric(const ray &r_in, const hit_record &rec, color &attenuation, ray &scattered,
                                  curandState &rand_state) const {
